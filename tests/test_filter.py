...
class KeywordFilterTests(unittest.TestCase):
    def test_matches_selected_title_and_content_case_insensitively(self) -> None:
        ...
        self.assertTrue(keyword_filter.matches(news_item(title="New gPt model", content="")))
        self.assertTrue(keyword_filter.matches(news_item(title="Other", content="An api update")))
        self.assertFalse(keyword_filter.matches(news_item(title="Other", content="Nothing relevant")))

    def test_short_keywords_use_ascii_boundaries(self) -> None:
        keyword_filter = KeywordFilter(("API", "IDE", "AI"), ("title",))
        self.assertFalse(keyword_filter.matches(news_item(title="rapid evidence chair")))
    ...
    def test_only_configured_fields_are_searched(self) -> None:
        ...
        self.assertFalse(keyword_filter.matches(news_item(title="Other", content="API")))
